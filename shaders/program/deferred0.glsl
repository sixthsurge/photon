...
--------------------------------------------------------------------------------
  ...
  const int colortex3Format  = RGBA8;          // full res    | animated overlays/vanilla sky (solid -> deferred3), bloomy fog amount (composite1 -> composite14)
  ...
  const int colortex13Format = RGBA16F;        // full res    | rain particles (translucent -> composite1), TAAU min color for AABB clipping
