...
bool raymarch_depth_buffer(
    ...
    for (int i = 0; i < refinement_step_count; ++i) {
        ...
        float depth = texelFetch(
                          SSRT_DEPTH_SAMPLER,
                          ivec2(hit_pos.xy * view_res * taau_render_scale),
                          0
        )
                          .x;

        if (depth < hit_pos.z &&
            abs(depth_tolerance - (hit_pos.z - depth)) < depth_tolerance) {
            hit_pos -= ray_step;
        } else {
            hit_pos += ray_step;
        }
    ...
    // No intersection if the binary search landed on a sky pixel or a hand
    // pixel
