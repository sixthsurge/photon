...
const Material water_material = Material(vec3(0.0), vec3(0.0), vec3(1.00), vec3(0.0), 0.002, 1.0, 0.0, 0.0, 0.08, false, false);
				...
				if (material_mask < 44u) { // 40-44
					...
					if (material_mask < 46u) { // 44-46
						if (material_mask == 44u) { // 44
							#ifdef HARDCODED_EMISSION
							// Beacon
							material.emission = 0.33 * step(0.2, hsl.z) * albedo_sqrt * step(max_of(abs(block_pos - 0.5)), 0.4);
							#endif
						} else { // 45
							#ifdef HARDCODED_EMISSION
							// End portal frame
						...
						if (material_mask == 46u) { // 46
							#ifdef HARDCODED_EMISSION
							// Sculk
							material.emission = 2.2 * material.albedo * isolate_hue(hsl, 200.0, 40.0) * smoothstep(0.5, 0.7, hsl.z);
							#endif
						} else { // 47
							#ifdef HARDCODED_EMISSION
							// Pink glow
