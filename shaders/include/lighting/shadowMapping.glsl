...
#ifdef SHADOW
float blockerSearch(sampler2D depthSampler, vec3 positionShadowScreen, vec3 positionShadowClip, float dither, out float sssDepth) {
	...
	for (uint i = 0; i < stepCount; ++i) {
		...
		float depth  = texelFetch(depthSampler, ivec2(coord * shadowMapResolution), 0).x;
		float weight = step(depth, positionShadowScreen.z);

		blockerDepth += weight * depth;
		weightSum    += weight;
		sssDepth     += max0(positionShadowScreen.z - depth);
	}
...
vec3 shadowSimple(vec3 positionShadowScreen) {
#ifdef SHADOW_COLOR
	float shadow0 = texture(shadowtex0, positionShadowScreen);

	if (shadow0 < 1.0 - eps) {
		float shadow1 = texture(shadowtex1, positionShadowScreen);
...
#else
	return vec3(texture(shadowtex1, positionShadowScreen));
#endif
}
...
vec3 shadowSoft(
...
#ifdef SHADOW_COLOR
		shadow += texture(shadowtex0, vec3(coord, positionShadowScreen.z));
#else
		shadow += texture(shadowtex1, vec3(coord, positionShadowScreen.z));
#endif
	}
	...
	for (uint i = 0; i < stepCount; ++i) {
		...
		float shadow = texture(shadowtex1, vec3(coord, positionShadowScreen.z));
...
vec3 calculateShadows(
	...
	positionShadowClip += SHADOW_BIAS * biasScale * sqr(distortionFactor) * shadowNormal;

	vec3 positionShadowScreen = distortShadowSpace(positionShadowClip, distortionFactor) * 0.5 + 0.5;
	...
	if (NoL < eps) {
		...
		return shadowSimple(positionShadowScreen) * distantShadow;
	}
...
#else
	float blockerDepth = blockerSearch(shadowtex0, positionShadowScreen, positionShadowClip, dither, sssDepth);
#endif
	...
	float penumbraRadius = blockerDepthToPenumbraRadius(positionShadowScreen.z, blockerDepth, cloudShadow);
