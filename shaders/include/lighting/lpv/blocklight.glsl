...
float lpv_distance_fade(vec3 scene_pos) {
    float distance_fade
        = 2.0 * max_of(abs(scene_pos / vec3(voxel_volume_size)));
...
#if COLORED_LIGHTS_VANILLA_LIGHTMAP_CONTRIBUTION_MODE == COLORED_LIGHTS_VANILLA_LIGHTMAP_CONTRIBUTION_MODE_SQUARED
        lpv_blocklight
