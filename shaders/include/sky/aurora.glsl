...
float aurora_shape(vec3 pos, float altitude_fraction) {
    const vec2 wind_0 = 0.00175 * vec2(0.7, 0.1);
    const vec2 wind_1 = 0.0028 * vec2(-0.1, -0.7);
    const float frequency = 0.000006 * AURORA_FREQUENCY;
    ...
    float worley_1 =
        texture(noisetex, pos.xz * frequency * 3.6 + wind_1 * frameTimeCounter).y;
...
vec3 aurora_color(vec3 pos, float altitude_fraction) {
    return mix(
        aurora_colors[0],
        aurora_colors[1],
        pow(clamp01(dampen(altitude_fraction)),0.7)
    );
}
...
vec3 draw_aurora(vec3 ray_dir, float dither) {
    ...
    const float volume_bottom = 1600.0;
    const float volume_top = 6000.0;
    ...
    return (0.0015 * AURORA_BRIGHTNESS) * emission * aurora_amount;
}
