...
// Crimson stem
                            ...
                            material.emission = albedo_sqrt *
                                linear_step(0.33, 0.5, hsl.z) * emission_amount;
#endif
                        }
                    }
                }
                ...
                if (material_mask < 36u) { // 32-36
                    if (material_mask < 34u) { // 32-34
                        if (material_mask == 32u) { // 32
                            ...
                            // Strong white light
                            material.emission =
                                1.00 * albedo_sqrt * (0.1 + 0.9 * cube(hsl.z));
#endif
                        } else { // 33
#ifdef HARDCODED_EMISSION
                            // Medium white light
                ...
                if (material_mask < 52u) { // 48-52
                    if (material_mask < 50u) { // 48-50
                        ...
                        if (material_mask == 50u) { // 50
                            ...
                            // Ochre froglight
                            material.emission =
                                0.40 * albedo_sqrt * (0.1 + 0.9 * cube(hsl.z));
#endif
                        }
                    }
                } else { // 52-56
