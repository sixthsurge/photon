...
const float nightVisionBrightness = 1.5;
...
vec3 getSceneLighting(
	...
	vec3 skylight  = mix(skyColors[0] * 1.3, horizonColor, horizonWeight);
		 ...
		 skylight *= 1.0 + 0.33 * clamp01(flatNormal.y) * (1.0 - shadows.x) * (timeNoon + timeMidnight);
	     skylight *= ao * pi;
#endif
	...
	lighting += CAVE_LIGHTING_I * directionalShading * ao * (1.0 - skylightFalloff);
	lighting += nightVision * nightVisionBrightness * directionalShading * ao;
