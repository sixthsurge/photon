...
const vec3[32] light_color = vec3[32](
	vec3(1.00, 1.00, 1.00) * 1.50, // Strong white light
	vec3(0.90, 0.30, 1.00) *  1.25, // Medium white light
	vec3(1.00, 1.00, 1.00) *  0.25, // Weak white light
	vec3(1.00, 0.55, 0.20) * 1.50, // Strong golden light
	vec3(1.00, 0.70, 0.35) *  0.3, // Medium golden light
	vec3(1.00, 0.40, 0.15) *  0.15, // Weak golden light
	vec3(1.00, 0.18, 0.10) *  1.75, // Redstone components
	vec3(1.00, 0.32, 0.00) * 8.0, // Lava
	vec3(1.00, 0.45, 0.10) *  3.25, // Medium orange light
	...
	vec3(0.0), // Unused
	vec3(0.0), // Unused
	vec3(0.60, 0.10, 1.00) * 8.0, // Nether portal
	vec3(0.0)  // End portal
);
